...
function Wizard({ init={}, onDone, onCancel }) {
  ...
  const pg = { fontFamily:"'DM Sans',sans-serif", background:T.bg, minHeight:"100vh", color:T.ink, padding:"48px 24px 60px", maxWidth:480, margin:"0 auto" };
  ...
  return (
    <div style={pg}><StepBar n={5} total={5}/><SL ch="Step 5 of 5 — How You Work"/>
      ...
      <div style={{ display:"flex", gap:10 }}><Btn variant="ghost" onClick={()=>setS(4)}>← Back</Btn><div style={{ flex:1 }}><Btn onClick={()=>onDone({name,title,skills,lanes:selLanes,customLanes,activeLanes:selLanes,answers,modeId:computed,focusStyle})} disabled={!focusStyle} full>Enter Mode OS →</Btn></div></div>
    </div>
  );
}
